...
class LocationProvider extends ChangeNotifier with ProviderHelperClass {
  ...
  checkLocationPermission(Completer<GoogleMapController>? mapController,
      GoogleMapController? _googleMapController,
      {bool requestService = false, bool openDeviceSetting = false}) async {
    ...
    if (network) {
      try {
        ...
        if (status == PermissionStatus.granted) {
          showLocReqBtn = false;
          checkServiceEnabled(mapController, _googleMapController,
                requestService: requestService);
          updateLoadState(LoaderState.loaded);
        } else if (status == PermissionStatus.denied) {
          showLocReqBtn = true;
          isLocationEnabled = false;
          locationMsg = Constants.permissionDenied;
          updateLoadState(LoaderState.loaded);
        } else if (status == PermissionStatus.permanentlyDenied) {
          isLocationEnabled = false;
          showLocReqBtn = true;
          locationMsg = Const.permissionDeniedPermanently;
  ...
  checkServiceEnabled(Completer<GoogleMapController>? mapController,
      GoogleMapController? _googleMapController,
    ...
    if (network) {
      try {
        if (!service) {
          isLocationEnabled = false;
          showLocReqBtn = false;
          locationMsg = Const.serviceDisabled;
          if (requestService) {
